...
AES-256 in GCM mode is used as the AEAD cipher to ensure the confidentility,
integrity and authenticity of the vault contents.
...
scrypt is used as the KDF to derive a key from a user-provided password, with
...
is known as __key wrapping__. This allows obtaining the master key by entering
...
Because of the use of an AEAD for encryption, the content and encrypted master
keys in the slots are checked for integrity and authenticity. The rest of the
file is not.
...
be incremented. The vault contents are stored under ``db``. The value depends on
...
There's also a Python script that can decrypt an Aegis vault given the password:
[scripts/decrypt.py](/scripts/decrypt.py).

### Header

The header starts with the list of ``slots``. It also contains the ``nonce`` and
``tag`` parameters that were produced during encryption, encoded in a
hexadecimal string. 
...
``key`` are encoded in a hexadecimal string and stored together. Slots also have
...
A random 256-bit ``salt`` is generated and passed to scrypt to protect against
rainbow table attacks. Its stored along with the ``N``, ``r`` and ``p``
parameters.
...
Each entry has a unique randomly generated ``UUID`` (version 4). as well as an
``issuer`` which represents the name of the entity that issued that. Entries can
also have an icon. These are JPEG's encoded in Base64 with padding. The ``info``
object holds information specific to the OTP type. The ``secret`` is encoded in
